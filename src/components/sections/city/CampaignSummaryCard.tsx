  ...
  return (
    <section className="bg-white rounded-2xl shadow-md border border-gray-100 overflow-hidden">
      <div className="flex flex-col md:flex-row">
        ...
        {/* 右テキスト */}
        <div className="flex-1 px-6 py-3 sm:p-8 space-y-5">
          {/* 概要テキスト */}
          <p className="text-sm text-gray-500 mb-0">{campaign.prefecture}{campaign.city}の対象店舗でのPayPay支払いで</p>

          {/* 最大ポイント */}
          <p className="text-2xl sm:text-3xl font-bold text-brand-primary mb-1">
            最大 <span className="text-red-600 text-4xl">{formatNumber(campaign.fullpoint)}</span> pt 還元
          </p>

          {/* 開催期間バッジ */}
          <div className="inline-block bg-red-500 text-white text-sm font-semibold px-4 py-1 rounded-full shadow-sm mt-1">
            {campaign.startDate} ～ {campaign.endDate}
          </div>

          {/* 上限2種 */}
          <div className="flex flex-wrap gap-3">
            <div className="bg-blue-50 border border-blue-200 rounded-full px-4 py-1.5 text-sm text-blue-800">
              期間上限：<span className="font-bold">{formatNumber(campaign.fullpoint)}</span> 円相当
            </div>
            <div className="bg-green-50 border border-green-200 rounded-full px-4 py-1.5 text-sm text-green-800">
              1回上限：<span className="font-bold">{formatNumber(campaign.onepoint)}</span> 円相当
            </div>
          </div>

          {/* 最も効率的な方法 */}
          <div className="bg-yellow-50 border border-yellow-200 rounded-xl px-5 py-4 mt-2">
